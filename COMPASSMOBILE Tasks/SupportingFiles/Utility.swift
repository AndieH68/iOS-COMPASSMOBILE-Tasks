    ...
    class func SendTaskDetails(_ viewController: UIViewController, HUD: MBProgressHUD?) -> (Bool, Int32) {
        ...
        if isRemoteAvailable {
            ...
            while (!noMore)
            {
                ...
                var PDASynchronisationPackage: String = "<PDASynchronisation>" + taskData + taskParameterData.replacingOccurrences(of: "\n", with: "") + "</PDASynchronisation>"
                
                PDASynchronisationPackage = PDASynchronisationPackage.GS1DataMatrixEscape().xmlSimpleEscape()
